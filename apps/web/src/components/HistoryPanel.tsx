...
export function HistoryPanel() {
  ...
  return (
    <GlassCard className="space-y-4">
      ...
      {!loading && history.length === 0 && <p className="text-sm text-slate-300">No completed downloads yet.</p>}

      <ul className="space-y-3">
        {history.slice(0, 8).map((item) => (
          <li key={item.id} className="rounded-xl border border-slate-500/30 bg-slate-900/50 px-3 py-2 text-sm text-slate-200">
