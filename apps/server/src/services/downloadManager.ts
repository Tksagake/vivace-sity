...
import { safeOutputTemplate } from '../utils/files.js'
...
export class DownloadManager {
  ...
  private buildArgs(task: QueueTask): string[] {
    ...
    const args: string[] = [
      ...
      this.ffmpegBin,
      '--output',
      safeOutputTemplate(this.downloadDir),
    ]
    ...
    try {
      ...
      const handleProgressLine = (line: string): void => {
        ...
        if (line.toLowerCase().includes('ffmpeg')) {
          this.emit(id, 'converting', { message: 'Converting with FFmpeg' })
        }
      }
      ...
      this.emit(id, 'completed', {
        ...
        filePath: path.resolve(this.downloadDir),
      })
